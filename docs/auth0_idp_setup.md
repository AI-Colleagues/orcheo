...
This guide shows how to configure Auth0 as the OAuth/OIDC identity provider for Orcheo Studio + backend when running the stack from `deploy/stack/docker-compose.yml`.

## Goal

Set `ORCHEO_AUTH_MODE=required` and allow authenticated Studio users to:
...
- A target Studio URL (for example `http://localhost:2026` in local dev).
...
3. Open the API `Permissions` tab and add:
   ...
   - `workflows:execute` — `Trigger workflow executions (including workflow-scoped ChatKit session startup in Studio).`
...
3. In **User Access permissions** for your Studio SPA app, explicitly add:
   - `workflows:read`
   - `workflows:execute`
   - `chatkit:session`

Critical: even when Studio requests these scopes, Auth0 will not include them in the access token unless API user access/client-grant permissions allow them for that application.
...
- `401 Missing required scopes`: requested scopes in Studio env do not match API permissions or token lacks those scopes.
