...
| `ORCHEO_CHATKIT_ATTACHMENT_BASE_URL`  | `http://credential-relay:9091`                                          | Child-facing base URL for ChatKit attachment downloads. The relay proxies these requests to the backend on the default network. |
| `ORCHEO_CHATKIT_ATTACHMENT_FORWARD_URL` | `http://backend:2025/api/chatkit/attachments`                          | Upstream attachment download URL read only by `credential-relay` on the control network. |
