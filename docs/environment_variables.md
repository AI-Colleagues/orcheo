...
| `ORCHEO_WORKFLOW_DEFINITION_MODE` | `unrestricted` | `restricted` or `unrestricted` | Selects how uploaded `workflow.py` files are ingested and executed. `restricted` compiles every upload to a frozen IR (no author code runs at ingestion) and runs `CodeNode` bodies in the MicroPython-WASM sandbox; `unrestricted` keeps today's in-process `load_graph_from_script` path and is **not tenant-safe**. In `restricted` mode built-in nodes that make outbound HTTP requests (e.g. `HttpRequestNode`) additionally apply an SSRF egress guard: only `http(s)` URLs whose host resolves to a public address are allowed, and requests to loopback/link-local/private/reserved/metadata addresses (including via redirects) are rejected. `restricted` mode also enforces a node capability policy at ingestion: **untrusted client uploads** may not use node types whose capability cannot be isolated from other tenants — browser (Playwright), communication (SMTP/webhook), database (`PostgresNode`, MongoDB), and file-backed document/dataset loader nodes are rejected. Curated first-party candidate onboarding is exempt (it passes `trusted_source=True`). The runtime default is `unrestricted`; `orcheo install` writes `restricted` for a trusted HTTPS backend and `unrestricted` for local-hosting or untrusted-HTTP deployments. See [Sandboxed Custom Workflows](sandboxed_custom_workflows.md) (`config/loader.py`, `graph/ir/definition_mode.py`, `graph/ir/node_policy.py`, `security/ssrf.py`, `cli/setup.py`). |
