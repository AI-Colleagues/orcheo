...
| Variable | Default | Valid values | Purpose |
| --- | --- | --- | --- |
| `ORCHEO_SANDBOX_DISABLED` | `false` unless the environment explicitly sets it to `true` | Boolean (`1/0`, `true/false`, etc.) | Disables workflow sandbox dispatch and agent-process sandboxing. The root `docker-compose.yml` sets this to `true` by default for local development. |
