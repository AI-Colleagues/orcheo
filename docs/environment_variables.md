...
| `ORCHEO_WORKFLOW_DEFINITION_MODE` | `unrestricted` | `restricted` or `unrestricted` | Selects how uploaded `workflow.py` files are ingested and executed. `restricted` compiles every upload to a frozen IR (no author code runs at ingestion) and runs `CodeNode` bodies in the MicroPython-WASM sandbox; `unrestricted` keeps today's in-process `load_graph_from_script` path and is **not tenant-safe**. In `restricted` mode built-in nodes that make outbound HTTP or SMTP requests additionally apply an SSRF egress guard: only targets whose host resolves to a public address are allowed, and requests to loopback/link-local/private/reserved/metadata addresses (including HTTP redirects) are rejected. `restricted` mode also rejects nodes whose registry metadata sets `restricted=True`; browser (Playwright), `PostgresNode`, and file-backed document/dataset loader nodes are currently marked restricted. MongoDB and guarded email/webhook nodes remain available. Curated first-party candidate onboarding is exempt from node rejection (it passes `trusted_source=True`). The runtime default is `unrestricted`; `orcheo install` writes `restricted` for a trusted HTTPS backend and `unrestricted` for local-hosting or untrusted-HTTP deployments. See [Sandboxed Custom Workflows](sandboxed_custom_workflows.md) (`config/loader.py`, `graph/ir/definition_mode.py`, `graph/ir/node_policy.py`, `security/ssrf.py`, `cli/setup.py`). |
...
| `VITE_ORCHEO_APPS_BASE_DOMAIN` | `beta.orcheo.cloud` | DNS hostname | Base domain Studio uses to display hosted-app URLs. Stack deployments populate it from `ORCHEO_APPS_BASE_DOMAIN`; set that operator-facing variable for self-hosted domains. |
...
Interactive `orcheo install` uses local defaults without additional Hosted Apps
questions when the resolved backend URL is loopback (`localhost`, a
`*.localhost` name, or a loopback IP address). When the resolved backend URL is
non-local, the installer asks whether to enable Hosted Apps and, when enabled,
prompts for the base domain and trusted proxy boundary. Hosted Apps defaults to
enabled. Existing values are offered as defaults during upgrades. TLS paths are
not prompted: CLI options or existing values win, followed by the fixed
`~/.orcheo/tls/` defaults shown below.

| Variable | Default | Valid values | Purpose |
| --- | --- | --- | --- |
| `ORCHEO_HOSTED_APPS_ENABLED` | `false` in the runtime; `true` for installed stacks | Boolean | Globally enables Hosted Apps. Interactive installs ask `Enable Hosted Apps?` with Yes as the default. |
| `ORCHEO_APPS_BASE_DOMAIN` | `apps.localhost` locally; public/backend hostname for new non-local installs | Bare DNS name such as `example.com` | Base domain beneath which one-label app aliases are served. Do not include a scheme or `*.` prefix. Prefer the DNS zone apex (for example, `orcheo.cloud`) so aliases such as `app-test-dashboard.orcheo.cloud` remain first-level names covered by Cloudflare Universal SSL. Existing values are preserved during upgrades. |
| `ORCHEO_HOSTED_APPS_WORKSPACE_ALLOWLIST` | Empty (all workspaces) | Comma-separated workspace IDs | Restricts Hosted Apps rollout to selected workspaces. It is preserved from an existing stack or set with `--hosted-apps-workspace-allowlist`; interactive setup does not prompt for it. |
| `ORCHEO_APP_TRUSTED_PROXY_CIDRS` | Empty locally; `172.16.0.0/12` installer default for non-local backends | Comma-separated IPv4/IPv6 CIDRs | Proxies from which the app gateway accepts forwarded client-address information. Prompted for non-local backends and must be configured together with `ORCHEO_APP_TRUSTED_PROXY_HOPS`. |
| `ORCHEO_APP_TRUSTED_PROXY_HOPS` | `0` locally; `1` installer default for non-local backends | Integer ≥ 0 | Fixed number of trusted forwarding hops before the app gateway. Prompted for non-local backends and must be greater than zero when trusted proxy CIDRs are set. |
| `ORCHEO_APP_TLS_METHOD` | `local` | `local`, `provided`, or `dns-01` | TLS provisioning mode for app hosts. The bundled installer selects `local` for loopback backends and `provided` for non-local backends; custom DNS-01 ingress is operator-managed. |
| `ORCHEO_APP_TLS_CERT_FILE` | `~/.orcheo/tls/apps-origin.pem` for new non-local installs | Readable PEM file path | Wildcard certificate for `*.ORCHEO_APPS_BASE_DOMAIN`. The installer does not prompt: `--app-tls-cert-file`, an existing value, or this default is validated and copied into the managed stack. |
| `ORCHEO_APP_TLS_KEY_FILE` | `~/.orcheo/tls/apps-origin-key.pem` for new non-local installs | Readable PEM file path | Matching private key. The installer does not prompt: `--app-tls-key-file`, an existing value, or this default is validated and copied with restricted permissions. |
| `ORCHEO_APP_GATEWAY_SECRET` | Auto-generated by `orcheo install` | Secret string of at least 32 characters | Dedicated identity shared by the app gateway and backend internal routes. Prompted only when `--manual-secrets` is used. |
| `ORCHEO_HOSTED_APPS_AUTO_ENABLE_RUNTIME` | `true` in bundled stacks | Boolean | Automatically enables the ephemeral Hosted Apps runtime for supported local or single-node deployments. |
| `ORCHEO_APP_BUNDLE_BACKEND` | `postgres` in bundled stacks | `postgres`, `filesystem`, or `s3` | Selects storage for staged archives and immutable deployment assets. Installed stacks persist package bytes in PostgreSQL so database backup/restore can move apps between machines. `filesystem` remains available for development and migration; S3 requires an external upload adapter. |
| `ORCHEO_APP_BUNDLE_FILESYSTEM_ROOT` | `/data/app-bundles` in bundled stacks | Directory path | Private filesystem root used when `ORCHEO_APP_BUNDLE_BACKEND=filesystem`. When PostgreSQL storage is selected, installed stacks scan this legacy location and idempotently migrate existing bundle objects into the database. |
| `ORCHEO_DEPLOYMENT_MODE` | `single-node` in bundled stacks | `local`, `single-node`, or an operator-defined external mode | Limits filesystem bundle storage to local and single-node deployments. |
| `ORCHEO_HOSTED_APPS_VALIDATION_QUEUE` | `hosted-app-validation` | Non-empty Celery queue name | Dedicated queue consumed by the Hosted Apps validation worker. It must not share the normal workflow queue. |
...
| Variable | Default | Valid values | Purpose |
| --- | --- | --- | --- |
| `REDIS_URL` | `redis://localhost:6379/0` | Redis connection URL | Broker URL for Celery task queue (`celery_app.py`). The desktop apps set this to their own bundled Redis instance on a private loopback port. |
| `ORCHEO_CRON_DISPATCH_INTERVAL` | `60` | Float (seconds), positive and finite | Interval at which Celery Beat and the in-process cron scheduler dispatch cron triggers (`celery_app.py`, `app/cron_scheduler.py`). Non-numeric, non-positive, and non-finite values (`nan`, `inf`) fall back to the default. |
...
| `ORCHEO_INPROCESS_CRON` | `true` | Boolean (`1/0`, `true/false`, `yes/no`, `on/off`) | Runs the cron dispatch loop inside the backend process, polling every `ORCHEO_CRON_DISPATCH_INTERVAL` seconds (`app/cron_scheduler.py`). On by default so schedules fire without Redis (desktop app, bare `uvicorn`). **Set to `false` wherever Celery Beat runs**, or every cron trigger dispatches twice; the bundled compose stacks already do. **Also set it to `false` when more than one backend process serves the same database** (multiple Uvicorn workers, or replicas behind a load balancer): each process runs its own dispatch loop guarded only by a process-local lock, so two can see the same occurrence as due and create duplicate runs. Only a single-process backend may leave it on. |
| `ORCHEO_INPROCESS_EXECUTION` | `true` | Boolean (`1/0`, `true/false`, `yes/no`, `on/off`) | Executes dispatched runs on the backend event loop instead of publishing them to Celery (`app/local_execution.py`). On by default so runs do not sit `pending` without Redis. Set to `false` wherever a Celery worker runs, so runs are queued to the worker fleet instead of the API process; the bundled compose stacks already do. Ignored inside Celery tasks, which always publish to the broker. |
...
| `ORCHEO_STACK_VERSION` | _unset_ | Stack release version string (for example `0.1.0` or `0.2.0-rc.1`) | Pins `orcheo install` to a specific `stack-v*` release when `--stack-version` is not provided. The installer records its resolved stable or staging version here. |
| `ORCHEO_STACK_IMAGE` | `ghcr.io/ai-colleagues/orcheo-stack:latest` | Container image reference | Runtime image used by `deploy/stack/docker-compose.yml` for backend/worker/celery-beat services. Versioned and staging installs pin it to the resolved stack version. |
| `ORCHEO_STUDIO_IMAGE` | `ghcr.io/ai-colleagues/orcheo-studio:latest` | Container image reference | Studio runtime image. Versioned and staging installs pin it to the same resolved version as `ORCHEO_STACK_IMAGE`. |
| `ORCHEO_APP_GATEWAY_IMAGE` | `ghcr.io/ai-colleagues/orcheo-app-gateway:latest` | Container image reference | Dedicated Hosted Apps delivery gateway image. Versioned and staging installs pin it to the same resolved version as `ORCHEO_STACK_IMAGE`. |
