...
class DataQualityNode(TaskNode):
    ...
    async def run(self, state: State, config: RunnableConfig) -> dict[str, Any]:
        ...
        if not source_units:
            source_units = get_units(state, self.result_keys)
        units, report = assess_quality(source_units)
        return {
            self.units_field: [u.model_dump(mode="json") for u in units],
            self.quality_report_field: report.model_dump(mode="json"),
