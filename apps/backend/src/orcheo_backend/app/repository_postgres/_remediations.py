"""PostgreSQL remediation candidate repository support."""
...
class WorkflowRemediationMixin(PostgresPersistenceMixin):
        ...
        *,
        ...
        async with self._lock:
            async with self._connection() as conn:
                ...
                if row is not None:
                    return self._parse_remediation_payload(row["payload"])
        ...
        *,
        ...
        async with self._lock:
            ...
            async with self._connection() as conn:
                ...
                if row is None:
                    return None
                candidate = self._parse_remediation_payload(row["payload"])
    ...
    async def list_remediation_candidates(
        ...
        *,
        ...
        if workflow_version_id is not None:
            clauses.append("workflow_version_id = %s")
        ...
        async with self._lock:
            ...
            return [self._parse_remediation_payload(row["payload"]) for row in rows]
    ...
    async def _get_remediation_locked(
        ...
        async with self._connection() as conn:
            cursor = await conn.execute(
                "SELECT payload FROM workflow_run_remediations WHERE id = %s",
                (str(remediation_id),),
            )
            row = await cursor.fetchone()
        if row is None:
    ...
    @staticmethod
    def _parse_remediation_payload(
        payload: dict[str, Any] | str,
    ) -> WorkflowRunRemediation:
        if isinstance(payload, str):
            return WorkflowRunRemediation.model_validate_json(payload).model_copy(
                deep=True
            )
        return WorkflowRunRemediation.model_validate(payload).model_copy(deep=True)
