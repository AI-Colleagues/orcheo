...
- **Type:** Enhancement
- **Summary:** Bundle Caddy into the standard Orcheo stack so self-hosted operators can expose Studio and Backend on a public HTTPS domain without requiring Cloudflare Tunnel.
...
| Studio Host Validation | `apps/studio/vite.config.ts` | Shaojie Jiang | Studio Vite configuration |
...
- Operators who need one public origin for Studio, backend APIs, and workflow WebSockets.
...
| Self-hosted operator | serve Studio, `/api/*`, and `/ws/*` from one public origin | I can avoid ad-hoc proxy config and reduce CORS complexity | P0 | Caddy routes browser traffic correctly for Studio, API, and workflow WebSockets |
...
- Serve Studio, backend APIs, and workflow WebSockets under one HTTPS origin.
...
- In bundled-ingress mode, Caddy is the only service that needs public `80/443` exposure. Backend and Studio remain private to the Docker network by default.
- Caddy must route:
  - `/` and SPA paths to Studio
...
- Stack configuration must set Orcheo’s public-origin settings correctly for bundled ingress, including:
  ...
  - Studio allowed hosts where relevant
...
- Studio: allowed-host behavior and public backend URL defaults.
- Docs/Developer Relations: self-hosted deployment guidance and tunnel-vs-ingress positioning.

## TECHNICAL CONSIDERATIONS
### Architecture Overview
Bundle Caddy as the public entrypoint for standard self-hosted stacks while keeping backend, Studio, Redis, Postgres, worker, and beat in the existing Compose-based topology. Caddy terminates TLS, serves the public hostname, and reverse-proxies HTTP and WebSocket traffic to the internal services.
...
- Default public routing model:
  ...
  - `wss://<host>/ws/...` -> backend
- Ensure the generated runtime configuration keeps Studio talking to the public backend origin and backend CORS aligned to that origin.
