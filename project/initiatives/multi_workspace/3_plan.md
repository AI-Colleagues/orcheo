...
SaaS clarification: workspace creation/joining is treated as onboarding or invite acceptance, not as a free-form action available from the main shell. The active workspace switcher should only enumerate memberships the user already has. Orcheo Vibe is also a workspace-bound path: external-agent auth, login state, and working directories must all stay inside the active workspace boundary.
