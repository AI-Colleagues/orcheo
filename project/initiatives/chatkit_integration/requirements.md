...
2. **Studio chat bubble** for authenticated creators to test the workflow inline.

Both surfaces must reuse a single ChatKit backend endpoint while enforcing the correct authentication path (public workflow id + optional OAuth vs. JWT) and respecting workflow publish status. The ChatKit widget code must be shared between public and Studio surfaces to minimize divergence.
Publishing, unpublishing, and rotating workflows are initiated through the Orcheo CLI and mirrored via the MCP server so human operators and AI assistants share the same UX until the Studio UI catches up.

## Goals
- Let workflow owners publish a workflow and share a public chat URL that works from the open internet.
- Provide Studio users with an in-editor chat bubble for rapid testing of the workflow currently being edited.
...
- Supporting multi-workflow sessions inside the Studio chat bubble.
- Implementing advanced moderation features beyond basic rate limiting/CAPTCHA.

## Personas
- **Workflow Creator (authenticated)**: logged-in Studio user editing workflows, needs quick testing.
...
| 3 | Creator | I am editing a workflow in Studio | Open a chat bubble scoped to that workflow using my auth context | I can validate behavior without leaving the editor |
...
- **Publish Workflow**
  - Introduce explicit "Publish" action (delivered via the CLI and mirrored through the MCP server, with Studio UX tracked as future work) that flips `workflow.is_public = true` and lets the owner opt into requiring OAuth login.
...
- **Studio Chat Bubble**
  ...
  - On open, Studio backend issues short-lived JWT (1–5 min) bound to user + workflow. Modal refreshes token as needed.
...
- **Shared ChatKit Widget**
  ...
  - Styling/theme hooks ensure Studio modal and public page just pass container props.
- **Backend Contract**
...
- Chat transcripts (public and Studio) persist indefinitely unless retention policy changes; stored alongside other ChatKit sessions.

## Success Metrics
- ≥80% of creators who publish a workflow can complete a public chat session without assistance.
- Studio chat bubble latency ≤ 2s p95 round-trip for first message after open.
